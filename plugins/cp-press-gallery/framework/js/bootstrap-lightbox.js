...
+function ($) { "use strict";
	...
	Lightbox.prototype.preloadSize = function(callback)
	{
		...
		{
			thumbHeight = 0;
			if(that.$element.find('.lightbox-image-wrapper').find('img').first().length > 0){
				thumbHeight = that.$element.find('.lightbox-image-wrapper').find('img').first().height();
				thumbHeight += 20;
			}
			...
			$image.css({
				...
				'height': preloader.height-(thumbHeight)
			})
